...
class ProcessRequirementsTests(unittest.TestCase):
    ...
    def test_spm(self):
        def generate_spm_configuration(config_file, spm_versions, matlab=True):
            ...
            config = {"builtin": {"persistent": False}}
            ...
            for version in spm_versions:
                ...
                config["builtin"].setdefault("spm", {})[fakespm_name] = fakespm_config
        ...
        # 1 - multiple spm config are valid
        generate_spm_configuration(config_file, spm_versions=("12", "12"))

        # Check that exception is raised
        with self.assertRaises(RuntimeError) as context:
            get_execution_context_for_config(
                config_file, "capsul.test.test_process_requirements.ProcessWithSpm12"
            )
        self.assertTrue(
            'Execution environment "builtin" has 2 possible configurations for module spm'
            == str(context.exception)
        )

        # 2 - no spm config defined correctly
        generate_spm_configuration(config_file, spm_versions=("8",))
        ...
        self.assertTrue(
            """Execution environment "builtin" has no valid configuration for module spm.
  - spm_8_0 is not valid for requirements: spm configuration does not match required version 12"""
        ...
        self.assertTrue(
            """Execution environment "builtin" has no valid configuration for module spm.
  - spm_8_0 is not valid for requirements: spm configuration does not match required version 12
  - spm_8_1 is not valid for requirements: spm configuration does not match required version 12"""
            == str(context.exception)
        )

        # 4 - matlab config not defined correctly
        generate_spm_configuration(config_file, spm_versions=("8", "12"), matlab=False)
        ...
        self.assertTrue(
            """Execution environment "builtin" has no valid configuration for module matlab."""
            == str(context.exception)
        )

        # 5 - spm standalone config defined correctly
        generate_spm_configuration(config_file, spm_versions=("8", "12"))

        context_dict = get_execution_context_for_config(
            config_file, "capsul.test.test_process_requirements.ProcessWithSpm12"
        )
        ...
        self.assertEqual(
            context_dict,
            {
                ...
                "matlab": {"mcr_directory": f"{self.tmp}/software/matlab"},
            },
        )
